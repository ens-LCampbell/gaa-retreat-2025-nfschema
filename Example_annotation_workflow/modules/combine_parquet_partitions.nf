...
process COMBINE_PARQUET_PARTITIONS {
    label 'local'
    tag "partitions-${gff3_type}:${species}"
    storeDir "${params.cacheDir}/${species}"
    
    input:
        // tuple val(species), val(common), val(feature_build), val(ens_version), path(partition_dir)
        tuple val(species), val(build_version), path(partition_dir)
        val(gff3_type)
        // tuple val(species), path(partition_dir)
